...
#Test Info
  ...
  it 'C13796 Allows a user to complete an eCOA survey that has been missed.', test_id: 'C13796' do
    test_rail_expected_steps(2)

    #Step1 Make a Post request on {{protocol}}{{url}}/v3/data_collection/flow_process_values
    test_rail_expected_result(1, "User can get 200 response with the new captured_value_group_id and a submitted_at value that is the past date and time which was provided in the request")
    #Super User Session
    ...
    test_rail_expected_result(2, "User can get 200 response and the submitted_at value that is the past date and time which was provided in the request is displayed")
    #GET request on {{protocol}}{{url}}/v3/data_collection/captured_value_groups
    datacollection_capturedvaluegroups = V3::DataCollection::CapturedValueGroups::Index.new(super_user_session.token, user_email, base_url)
    ...
    match = resp_json['data'].any? {|data| data.dig('attributes', 'submitted_at') == "#{text}"}
