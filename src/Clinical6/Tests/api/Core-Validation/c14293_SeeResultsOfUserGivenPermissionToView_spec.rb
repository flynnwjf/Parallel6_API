...
#Main User(s)
  ...
  it 'C14293 Allows a users to see the results of another user they are given permission to view.', test_id: 'C14293' do
    test_rail_expected_steps(3)

    #Step1 Make a Post request on {{protocol}}{{url}}/v3/data_collection/flow_process_values
    test_rail_expected_result(1, "The flow is submitted and the data is saved into the system")
    #Super User Session
    ...
    test_rail_expected_result(2, "All captured data is displayed")
    #Get request {{protocol}}{{url}}/v3/data_collection/captured_value_groups
    datacollection_capturedvaluegroups = V3::DataCollection::CapturedValueGroups::Index.new(mobile_user_session.token, user_email, base_url)
    resp_code = datacollection_capturedvaluegroups.response_mobile.code
    test_rail_result(2, "datacollection_capturedvaluegroups header: #{datacollection_capturedvaluegroups.response_mobile.headers}")
    test_rail_result(2, "datacollection_capturedvaluegroups body: #{datacollection_capturedvaluegroups.response_mobile.body}")
    expect(resp_code).to eq 200
    ...
    test_rail_expected_result(3, "The user receives an error message (no permissions)")
    ...
    #Get request {{protocol}}{{url}}/v3/data_collection/captured_value_groups
    datacollection_capturedvaluegroups = V3::DataCollection::CapturedValueGroups::Index.new(unauthorized_user_session.token, unauthorized_user_email, base_url)
