...
#Test Info
  ...
  it 'C13812 A site user can edit an existing CRF in the system on behalf of another user', test_id: 'C13812' do
    test_rail_expected_steps(4)

    #Step1 Make a Post request on {{protocol}}{{url}}/v3/data_collection/flow_process_values
    test_rail_expected_result(1, "User receives response: 200, The free form text data is submitted and captured.")
    #Super User Session
    ...
    #Step2 Make a GET request on {{protocol}}{{url}}/v3/data_collection/flow_process_values/{ID}
    test_rail_expected_result(2, "The user is able to view the submitted data.")
    ...
    test_rail_result(2, "datacollection_flowprocessvalues header: #{datacollection_flowprocessvalues.response.headers}")
    test_rail_result(2, "datacollection_flowprocessvalues body: #{datacollection_flowprocessvalues.response.body}")
    expect(resp_code).to eq 200
    test_rail_result(2, "getting data collection flow process values response code: #{resp_code}", "pass")
    expect(resp_json.dig('data', 'attributes', '22')).to eq text
    test_rail_result(2, "submitted data is in response: #{text}", "pass")

    #Step3 Make a Post request on {{protocol}}{{url}}/v3/data_collection/flow_process_values
    test_rail_expected_result(3, "The submitted data is updated, including the reason for change.")
    #Post request on {{protocol}}{{url}}/v3/data_collection/flow_process_values
    text = "reason 2"
    datacollection_flowprocessvalues_create = V3::DataCollection::FlowProcessValues::Create.new(super_user_session.token, user_email, base_url, flow_id, mobile_user_id, group_id, text)
    resp_code = datacollection_flowprocessvalues_create.response.code
    ...
    expect(resp_code).to eq 200
    test_rail_result(3, "creating data collection flow process values response code: #{resp_code}", "pass")

    #Step4 Make a GET request on {{protocol}}{{url}}/v3/data_collection/flow_process_values/{ID}
    test_rail_expected_result(4, "The user is able to view the submitted data.")
    #GET request on {{protocol}}{{url}}/v3/data_collection/flow_process_values/{ID}
    datacollection_flowprocessvalues = V3::DataCollection::FlowProcessValues::Show.new(super_user_session.token, user_email, base_url, group_id)
    resp_code = datacollection_flowprocessvalues.response.code
    resp_json = JSON.parse(datacollection_flowprocessvalues.response.body)
    test_rail_result(4, "datacollection_flowprocessvalues header: #{datacollection_flowprocessvalues.response.headers}")
    test_rail_result(4, "datacollection_flowprocessvalues body: #{datacollection_flowprocessvalues.response.body}")
    expect(resp_code).to eq 200
    test_rail_result(4, "getting data collection flow process values response code: #{resp_code}", "pass")
    expect(resp_json.dig('data', 'attributes', '22')).to eq text
    test_rail_result(4, "submitted data is in response: #{text}", "pass")
  end
