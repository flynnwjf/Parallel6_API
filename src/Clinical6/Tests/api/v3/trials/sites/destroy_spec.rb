...
describe 'Delete V3/trials/sites/:id/destroy' do
#Environment Info
...
#Test Info
  let(:testname) { "trials_sites_destroy" }
  let(:test_data) { DataHandler.get_test_data(testname) }
#Requests
  let(:token) { V3::Users::Session::Create.new(user_email, user_password, base_url).token }
  let(:trials_sites_destroy) { V3::Trials::Sites::Destroy.new(token, user_email, base_url, id) }

  context 'with valid user' do
    let(:id) {  V3::Trials::Sites::Create.new(token, user_email, base_url, type, name).id }
    it 'returns 204 status code' do
      expect(trials_sites_destroy.response.code).to eq 204
      #cleanup
  ...
  context 'with invalid id' do
    ...
    it 'returns 404 error & record not found message in body' do
      expect(trials_sites_destroy.response.code).to eq 404
      expect(trials_sites_destroy.response.body).to match /Record Not Found/
      #cleanup
      expect(V3::Users::Session::Delete.new(token, user_email, base_url).response.code).to eq 204
    end
  end
  ...
  context 'with unauthorized user' do
    ...
    let(:id) {  V3::Trials::Sites::Create.new(token, user_email, base_url, type, name).id }
    let(:trials_sites_destroy) { V3::Trials::Sites::Destroy.new(unauthorized_user_token, unauthorized_email, base_url, id) }
    it 'returns 403 error' do
      expect(trials_sites_destroy.response.code).to eq 403
      #cleanup
  ...
  context 'with invalid user' do
    ...
    let(:trials_sites_destroy) { V3::Trials::Sites::Destroy.new(token, invalid_user, base_url, "1") }
    it 'returns 401 error' do
      expect(trials_sites_destroy.response.code).to eq 401
      expect(trials_sites_destroy.response.body).to match /Authentication Failed/
      #cleanup
      expect(V3::Users::Session::Delete.new(token, user_email, base_url).response.code).to eq 204
    end
  end
