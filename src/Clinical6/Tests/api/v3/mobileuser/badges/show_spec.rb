...
describe 'Get V3/mobile_users/:mobile_user_id/badges/:id/show' do
#Environment Info
...
#Test Info
  let(:testname) { "mobileuser_badges_show" }
  let(:test_data) { DataHandler.get_test_data(testname) }
#Requests
  let(:token) { V3::Users::Session::Create.new(user_email, user_password, base_url).token }
  let(:mobileuser_badges_show) { V3::MobileUser::Badges::Show.new(token, user_email, base_url, mobile_user_id, id) }
  ...
  context 'with valid user' do
    ...
    it 'returns 200 status code' do
      expect(mobileuser_badges_show.response.code).to eq 200
      expect(JSON.parse(mobileuser_badges_show.response).dig('data', 'id')).to eq id
      #cleanup
      expect(V3::MobileUser::Badges::Destroy.new(token, user_email, base_url, mobile_user_id, id).response.code).to eq 204
  ...
  context 'with invalid id' do
    ...
    it 'returns 404 error & record not found message in body' do
      expect(mobileuser_badges_show.response.code).to eq 404
      expect(mobileuser_badges_show.response.body).to match /Record Not Found/
      #cleanup
      expect(V3::Users::Session::Delete.new(token, user_email, base_url).response.code).to eq 204
    end
  end
  ...
  context 'with unauthorized user' do
    ...
    it 'returns 403 error' do
      expect([403, 404]).to include (mobileuser_badges_show.response.code)
  ...
  context 'with invalid user' do
    ...
    it 'returns 401 error' do
      expect(mobileuser_badges_show.response.code).to eq 401
      expect(mobileuser_badges_show.response.body).to match /Authentication Failed/
    end
  end
