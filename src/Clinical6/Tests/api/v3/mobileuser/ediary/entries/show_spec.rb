...
describe 'Get V3/mobile_users/:id/ediary/entries' do
#Environment Info
...
#Requests
  ...
  let(:mobileuser_ediary_entries_show) { V3::MobileUser::Ediary::Entries::Show.new(token, user_email, base_url, id) }
  ...
  context 'with invalid id' do
    ...
    it 'returns 404 error & record not found message in body' do
      expect(mobileuser_ediary_entries_show.response.code).to eq 404
      expect(mobileuser_ediary_entries_show.response.body).to match /Record Not Found/
    end
  end
  ...
  context 'with invalid user' do
    ...
    it 'returns 401 error & Authentication Failed message in body' do
      expect(mobileuser_ediary_entries_show.response.code).to eq 401
      expect(mobileuser_ediary_entries_show.response.body).to match /Authentication Failed/
    end
  end
