...
describe 'Get V3/consent/approvers/:id/show' do
#Environment Info
...
#Requests
  ...
  let(:consent_approvers_show) { V3::Consent::Approvers::Show.new(token, user_email, base_url, id) }
  ...
  context 'with invalid id' do
    ...
    it 'returns 404 error & record not found message in body' do
      ...
      expect(consent_approvers_show.response.body).to match /Record Not Found/
      #cleanup
      expect(V3::Users::Session::Delete.new(token, user_email, base_url).response.code).to eq 204
    end
  end
  ...
  context 'with invalid user' do
    ...
    let(:id) { "1" }
    let(:consent_approvers_show) { V3::Consent::Approvers::Show.new(token, invalid_user, base_url, id) }
    it 'returns 401 error' do
      expect(consent_approvers_show.response.code).to eq 401
      expect(consent_approvers_show.response.body).to match /Authentication Failed/
      #cleanup
      expect(V3::Users::Session::Delete.new(token, user_email, base_url).response.code).to eq 204
    end
  end
