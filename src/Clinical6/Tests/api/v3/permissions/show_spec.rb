...
describe 'Get V3/permissions/:id/show' do
#Environment Info
...
#Requests
  ...
  let(:permissions_show) { V3::Permissions::Show.new(token, user_email, base_url, id) }

  context 'with valid user' do
    it 'returns 200 status code' do
      expect(permissions_create.response.code).to eq 201
      expect(permissions_show.response.code).to eq 200
      expect(JSON.parse(permissions_show.response).dig('data', 'id')).to eq id
      #cleanup
  ...
  context 'with invalid user' do
    ...
    let(:permissions_show) { V3::Permissions::Show.new(token, invalid_user, base_url, id) }
    it 'returns 401 error' do
      expect(permissions_create.response.code).to eq 201
      expect(permissions_show.response.code).to eq 401
      expect(permissions_show.response.body).to match /Authentication Failed/
      #cleanup
      expect(V3::Permissions::Destroy.new(token, user_email, base_url, id).response.code).to eq 204
      expect(V3::Users::Session::Delete.new(token, user_email, base_url).response.code).to eq 204
    end
  ...
  context 'with invalid id' do
    ...
    it 'returns 404 error & record not found message in body' do
      expect(permissions_create.response.code).to eq 201
      expect(permissions_show.response.code).to eq 404
      expect(permissions_show.response.body).to match /Record Not Found/
      #cleanup
